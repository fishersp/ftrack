...
def upload_to_n8n(file_path: str, meta: dict = None):
    ...
    try:
        ...
        if response.status_code == 200:
            log.info("✅ Файл + данные успешно отправлены в n8n.")
        else:
            log.warning("⚠️ Ошибка при отправке в n8n: %s | %s", response.status_code, response.text)
    except Exception:
...
def main():
    ...
    log.info("=== Бот с уведомлением в Telegram через n8n запущен ===")
    app.run_polling()
