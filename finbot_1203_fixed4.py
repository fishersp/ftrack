...
import configparser
import pytz
# --- Патчим tzlocal, чтобы возвращал pytz‑таймзону -----------------
import tzlocal
...
tzlocal.get_localzone = _get_localzone
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔄 Обрабатываю: {category}, {amount:.0f}₽{f', {comment}' if comment else ''}..."
    )
...
# main
def main():
    tz = pytz.timezone("Europe/Moscow")    app = (
        ApplicationBuilder()
