# telegram_fin_tracker_n8n.py — Финансовый бот с выгрузкой на GDrive через n8n
# --------------------------------------------------------------------------
...
def upload_to_n8n(file_path: str):
    if not os.path.exists(file_path):
        log.warning("Файл не найден для отправки в n8n: %s", file_path)
        return
    try:
        ...
        log.exception("Ошибка при загрузке файла в n8n")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await update.message.reply_text(
            "Не удалось записать расход. Проверь Excel-файл."
        )
...
def main():
    ...
    log.info("=== Бот с интеграцией n8n запущен ===")
    app.run_polling()
