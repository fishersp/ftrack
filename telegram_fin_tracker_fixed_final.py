...
def get_drive_sheet(sheet_name: str) -> pd.DataFrame:
    """Скачиваем актуальный Excel через n8n и возвращаем указанный лист"""
...
def upload_to_n8n(file_path: str, meta: dict | None = None):
    """Отправляем файл + мета‑данные в n8n (async)"""
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            # stream=True и без чтения ответа → не ловим ChunkedEncodingError
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await update.message.reply_text("Не удалось записать расход или отправить файл.")
