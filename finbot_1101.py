...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Загружаем локальный Excel-файл
    excel_path = config["bot"].get("EXCEL_PATH", "Финансовый_план_и_долги (8).xlsx")
    wb = load_workbook(excel_path)
    ws = wb["Трекер расходов"] if "Трекер расходов" in wb.sheetnames else wb.active

    # Добавляем строку
    ws.append([date_str, category, amount, comment, "да"])
    ...
    files = {
        "file": ("Финансовый_план.xlsx", output, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    }

    log.info(f"Отправляю файл с записью: {payload}")
    ...
    if response.status_code == 200:
        ...
        log.error(f"Ошибка отправки файла: {response.status_code} - {response.text[:200]}")
        await update.message.reply_text("❌ Не удалось отправить файл в n8n.")
